package com.posthub.controllers;

import com.posthub.entities.Post;
import com.posthub.services.PostService;
...
//    updatePosts
    ...
    public ResponseEntity<String> checkPosts() {
        boolean contains = this.postService.checkPosts();
        if (!contains) {
            return new ResponseEntity<>("DB is empty", HttpStatus.OK);
        }
        return new ResponseEntity<>("DB is not empty", HttpStatus.OK);
    }
}
